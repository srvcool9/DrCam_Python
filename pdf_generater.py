...
def register_pdf_route(app):
 ...
 def handle_selected_images():
     try:
         ...
         pdf_path = copy_pdf_to_temp(preview_path, temp_pdf_path)
 ...
 def finalize_pdf():
     ...
     # Append preview PDF images (simple re-gen for now)
     pdf.cell(0, 10, "Attached Medical Images", ln=True)

     pdf.output(final_path)

     return send_file(final_path, as_attachment=True, download_name="Final_Report.pdf")
